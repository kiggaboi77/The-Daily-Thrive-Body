...
hero = (SRC / "hero.svg").read_text()
hero_uri = "data:image/svg+xml," + urllib.parse.quote(re.sub(r"\s+", " ", hero), safe=" =:/,;.-_()'")
skin = (SRC / "skin.css").read_text().replace("@HERO@", hero_uri)
...
cards = "".join(f"""<article class='card reveal'><a class='card-thumb' href='post.html' tabindex='-1'><span aria-hidden='true' class='card-thumb-fallback'>&#9733;</span></a>
<div class='card-body'><a class='card-label' href='#'>{l}</a><h2 class='card-title'><a href='post.html'>{t}</a></h2><p class='card-excerpt'>{e}</p><span class='card-meta'><time>September {20 - i}, 2026</time></span></div></article>""" for i, (l, t, e) in enumerate(posts))
...
index_html = (head.replace("{title}", "Preview – Home") + "<body class='is-home'>" + render(header, "", "") + "<main id='main-content'>"
    + render(home_top, "", "") + f"<div class='container main-wrap'>{grid}</div>" + topics + cta + "</main>" + render(footer, "", "")
...
post_main = f"""<div class='container main-wrap'><article class='post-single'><header class='post-hero'><p class='eyebrow'><a href='#'>Money</a></p>
...
<div class='blog-pager' id='blog-pager'><a class='blog-pager-newer-link' href='#'>Newer post</a><a class='blog-pager-older-link' href='#'>Older post</a></div></div>"""
